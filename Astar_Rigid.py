...
def Map(x,y,resolution,d=radius+clearance):
	...
	# # Polygon 1
	x_pts = np.array([75,100,75,50,75])/resolution
	y_pts = np.array([120,150,185,150,120])/resolution
	...
	# Polygon 2 
	x_pts = np.array([50,75,25,20])/resolution
	y_pts = np.array([150,185,185,120])/resolution
	l5 = (y-y_pts[0]) >= ((y_pts[1]-y_pts[0])/(x_pts[1]-x_pts[0]))*(x-x_pts[0]) - d/resolution*(1+math.sqrt(((y_pts[1]-y_pts[0])/(x_pts[1]-x_pts[0]))**2))  
	l6 = (y-y_pts[1]) <= 0 + d/resolution
	l7 = (y-y_pts[2]) <= ((y_pts[3]-y_pts[2])/(x_pts[3]-x_pts[2]))*(x-x_pts[2]) + d/resolution*(1+math.sqrt(((y_pts[3]-y_pts[2])/(x_pts[3]-x_pts[2]))**2))
	l8 = (y-y_pts[3]) >= ((y_pts[0]-y_pts[3])/(x_pts[0]-x_pts[3]))*(x-x_pts[3]) - d/resolution*(1+math.sqrt(((y_pts[0]-y_pts[3])/(x_pts[0]-x_pts[3]))**2))
	 
	if (l5 and l6 and l7 and l8):
		q = 1
...
def obstacle_disp_pts():
	...
	circle_pts = [225,50,25]

	polygon_pts =  np.array([[20,120],[25,185],[75,185],[100,150],[75,120],[50,150]])
	polygon_pts[:,1] = (200 - polygon_pts[:,1])

	rhombus_pts = np.array([[225,10],[200,25],[225,40],[250,25]])
	rhombus_pts[:,1] = (200 - rhombus_pts[:,1])

	ellipse_pts = np.array([110,80,80,40])
	...
	Y = 200 -  np.array([30+10*math.sin(math.radians(60)), 30+75*math.sin(math.radians(30))+10*math.sin(math.radians(60)), 30+75*math.sin(math.radians(30)), 30])
	rect_pts = np.column_stack((X,Y))

	return circle_pts,polygon_pts,rhombus_pts, ellipse_pts, rect_pts
...
# Workspace  defined
if (Map(goal[0],goal[1],resolution) or Map(start[0],start[1],resolution)):
...
while not done:
	...
	# To display obstacles
	circle_pts,polygon_pts,rhombus_pts, ellipse_pts, rect_pts = obstacle_disp_pts()
	pygame.draw.circle(screen, lime, (circle_pts[0]*k,circle_pts[1]*k), circle_pts[2]*k)
	pygame.draw.polygon(screen,lime,k*polygon_pts)
	pygame.draw.polygon(screen,lime,k*rhombus_pts)
	pygame.draw.ellipse(screen,lime,k*ellipse_pts)
	pygame.draw.polygon(screen,lime,k*rect_pts)
